...
async def next_page(bot, query):
    ...
    btn.insert(0, [
        InlineKeyboardButton("❗ How to Download ❗", url=f"https://telegram.dog/Central_tutorial/22")
    ])
    btn.insert(0, [
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        await query.message.edit_text(
            text=script.STATUS_TXT.format(total, users, chats, round(used_dbSize, 2), round(free_dbSize, 2), cpu_usage, ram_usage, disk_usage, currentTime),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
